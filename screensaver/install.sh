...
[[ -x /usr/bin/python3 && -s $source_dir/controller.py && -s $source_dir/scenes/04-spirit-lanterns.txt ]] || { echo "The synchronized controller, Python, or scenes are missing." >&2; exit 1; }
if [[ -e $target || -L $target ]]; then
  ...
  if grep -q '^# Violet Oni:' "$target" && grep -q 'engine=/usr/bin/ttfx' "$target"; then
    ...
    echo "Previous helper saved at: $backup"
    exit 0
  fi
